...
-- ────────────────────────────────────────────────────────────────────────────
-- Migrations applied after the 2026-08-18 production dump.
--
-- This file is a dump of production D1, regenerated by exporting
-- `sqlite_master`. The dump reflects what was in production at that moment;
-- migrations applied AFTER the dump date are listed here so a fresh local
-- database arrives at the same shape without needing to replay every
-- migration file. Add new ALTERs and CREATE TABLEs in this section as the
-- migrations land in production, and remove them when the next dump
-- supersedes them.
-- ────────────────────────────────────────────────────────────────────────────

-- Migration 021 — cashdrawers.closed_at (Z-report uses this column).
-- See migrations/021-drawer-closed-at.sql.
ALTER TABLE cashdrawers ADD COLUMN closed_at TEXT;

-- Migration 022 — alerts table + indexes.
-- See migrations/022-alerts.sql.
